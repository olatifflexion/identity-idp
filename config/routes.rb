Rails.application.routes.draw do
  ...
  SamlEndpoint.suffixes.each do |suffix|
    ...
    match "/api/saml/auth#{suffix}" => 'saml_idp#auth', via: %i[get]
    # JS-driven POST redirect route to preserve existing session
    match "/api/saml/auth#{suffix}" => 'saml_post#auth', via: %i[post]
    # actual SAML handling POST route
    match "/api/saml/authpost#{suffix}" => 'saml_idp#auth', via: %i[post]
  end

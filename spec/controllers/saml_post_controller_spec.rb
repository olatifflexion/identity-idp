...
describe SamlPostController do
  describe 'POST /api/saml/auth' do
    ...
    it 'leaves the session cookie alone' do
      cookies[:_upaya_session] = 'abc123'

      post :auth

      expect(cookies[:_upaya_session]).to eq('abc123')
    end

    it 'does not set a new session cookie' do
      post :auth

      expect(cookies[:_upaya_session]).to be_nil
    end
  end
end
